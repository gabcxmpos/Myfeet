...
-- PASSO 2: Adicionar o valor "devoluções" ao enum user_role
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 
        FROM pg_enum 
        WHERE enumlabel = 'devoluções' 
        AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'user_role')
    ) THEN
        -- Adicionar o valor ao enum
        -- Nota: Em algumas versões do PostgreSQL, isso precisa ser executado separadamente
        EXECUTE 'ALTER TYPE user_role ADD VALUE ''devoluções''';
        RAISE NOTICE '✅ Valor "devoluções" adicionado ao enum user_role';
    ELSE
        RAISE NOTICE 'ℹ️ Valor "devoluções" já existe no enum user_role';
    END IF;
EXCEPTION
...
-- e atualizá-la para incluir 'devoluções'
DO $$
DECLARE
...
BEGIN
    ...
    LOOP
        -- Se a constraint não incluir 'devoluções', vamos recriá-la
        IF constraint_rec.constraint_def NOT LIKE '%devoluções%' THEN
            -- Remover a constraint antiga
            EXECUTE format('ALTER TABLE public.app_users DROP CONSTRAINT IF EXISTS %I', constraint_rec.conname);
            
            -- Criar nova constraint incluindo 'devoluções'
            ...
            IF NOT EXISTS (
                ...
                RAISE NOTICE '✅ Constraint atualizada para incluir "devoluções"';
            END IF;
        ELSE
            RAISE NOTICE 'ℹ️ Constraint já inclui "devoluções"';
        END IF;
    END LOOP;
...
SELECT 
    ...
    'Verifique se o valor "devoluções" foi adicionado corretamente' AS mensagem,
    'Agora você pode criar usuários com o perfil "devoluções"' AS proximo_passo;

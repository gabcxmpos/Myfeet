...
const StoreGerencialChecklist = ({ storeId }) => {
  ...
  useEffect(() => {
    const loadFromDatabase = async () => {
      ...
      try {
        ...
        if (dayChecklist && dayChecklist.gerencialTasks) {
          setLocalChecklist(dayChecklist.gerencialTasks);
          console.log('✅ [StoreGerencialChecklist] Dados carregados do banco:', dayChecklist.gerencialTasks);
        } else {
          ...
          console.log('🔄 [StoreGerencialChecklist] Estado inicializado do contexto:', storeChecklist);
        }
      } catch (error) {
        console.error('❌ [StoreGerencialChecklist] Erro ao carregar do banco:', error);
  ...
  useEffect(() => {
    ...
    if (contextStr !== localStr && Object.keys(storeChecklist).length > 0) {
      setLocalChecklist(storeChecklist);
      console.log('🔄 [StoreGerencialChecklist] Estado sincronizado do contexto externo:', storeChecklist);
    }
  }, [checklist[storeId]?.gerencialTasks, storeId]);
  ...
  useEffect(() => {
    const reloadFromDatabase = async () => {
      ...
      try {
        ...
        if (dayChecklist && dayChecklist.gerencialTasks) {
          const dbStr = JSON.stringify(dayChecklist.gerencialTasks);
          const localStr = JSON.stringify(localChecklist);
          
          if (dbStr !== localStr) {
            setLocalChecklist(dayChecklist.gerencialTasks);
            console.log('🔄 [StoreGerencialChecklist] Dados sincronizados do banco:', dayChecklist.gerencialTasks);
          }
        }
      } catch (error) {
        console.error('❌ [StoreGerencialChecklist] Erro ao sincronizar do banco:', error);
      }
    };
  ...
  const handleCheckChange = useCallback((taskId, checked) => {
    console.log('🔄 [StoreGerencialChecklist] Atualizando checklist:', { storeId, taskId, checked });
    ...
    updateGerencialChecklist(storeId, taskId, checked)
      .then(() => {
        console.log('✅ [StoreGerencialChecklist] Checklist salvo no banco com sucesso');
        isUpdatingRef.current = false;
      })
      .catch((error) => {
        console.error('❌ [StoreGerencialChecklist] Erro ao salvar no banco:', error);
        setLocalChecklist(previousState);
        isUpdatingRef.current = false;
      });
  }, [storeId, updateGerencialChecklist, localChecklist]);

  // Recarregar do banco periodicamente e quando checklist mudar no contexto
  useEffect(() => {
    const reloadFromDatabase = async () => {
      if (!storeId) return;
      ...
      try {
        ...
        if (dayChecklist && dayChecklist.gerencialTasks) {
          ...
          if (dbStr !== localStr) {
            setLocalChecklist(dayChecklist.gerencialTasks);
            console.log('🔄 [StoreGerencialChecklist] Dados recarregados do banco:', dayChecklist.gerencialTasks);
          }
        }
      } catch (error) {
        console.error('❌ [StoreGerencialChecklist] Erro ao recarregar do banco:', error);
      }
    };
    
    reloadFromDatabase();
  }, [checklist[storeId]?.gerencialTasks, storeId]); // Recarregar quando checklist mudar no contexto
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        ...
        console.log('✅ [StoreGerencialChecklist] Histórico carregado:', history?.length || 0);
      } catch (error) {
        console.error('❌ [StoreGerencialChecklist] Erro ao carregar histórico:', error);
      } finally {
...
export default StoreGerencialChecklist;

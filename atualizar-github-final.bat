@echo off
REM Script Batch para atualizar GitHub com todas as correções finais
REM Execute este script na raiz do projeto

echo.
echo ========================================
echo  ATUALIZAR GITHUB - VERSÃO FINAL
echo ========================================
echo.
...
REM 1. Devoluções - Checkbox "Não possui NF" + Correção SEM_NF
git add src/pages/ReturnsManagement.jsx
echo   [OK] ReturnsManagement.jsx (Checkbox 'Nao possui NF' + correcao SEM_NF)

REM 2. Supabase Service - Correção createReturn
git add src/lib/supabaseService.js
echo   [OK] supabaseService.js (Correcao createReturn com SEM_NF)

REM 3. Header - Botão hamburger corrigido
git add src/components/Header.jsx
echo   [OK] Header.jsx (Botao hamburger corrigido)

REM 4. Sidebar - Agenda de treinamento para supervisores
git add src/components/Sidebar.jsx
echo   [OK] Sidebar.jsx (Agenda de treinamento para supervisores)

REM 5. MainLayout - Toggle da sidebar
git add src/components/MainLayout.jsx
echo   [OK] MainLayout.jsx (Toggle da sidebar corrigido)

REM 6. MenuVisibilitySettings - Menu de visibilidade funcional
git add src/pages/MenuVisibilitySettings.jsx
echo   [OK] MenuVisibilitySettings.jsx (Menu de visibilidade corrigido)

REM 7. DataContext - updateMenuVisibility melhorado
...
git commit -m "fix: Corrigir menu de visibilidade, botao hamburger, checkbox 'Nao possui NF' e erro de constraint" -m "- Corrigido menu de visibilidade com sincronizacao de estado" -m "- Adicionado checkbox 'Nao possui NF' nas devolucoes pendentes" -m "- Corrigido erro de constraint NOT NULL usando 'SEM_NF' em vez de null" -m "- Corrigido botao hamburger para abrir/fechar sidebar corretamente" -m "- Adicionado Agenda de Treinamentos para supervisores" -m "- Melhorado toggle da sidebar em desktop e mobile" -m "- Melhorado updateMenuVisibility com atualizacao imediata de estado"

...
-- PASSO 1: Verificar se existe tabela "users" no PUBLIC (ISSO É O PROBLEMA!)
DO $$
DECLARE
...
BEGIN
  ...
  IF public_users_exists THEN
    ...
    RAISE NOTICE 'Total de registros: %', public_users_count;
    RAISE NOTICE '';
    RAISE NOTICE 'SOLUÇÃO:';
    IF public_users_count = 0 THEN
      RAISE NOTICE 'A tabela está vazia. Vamos removê-la...';
      DROP TABLE IF EXISTS public.users CASCADE;
      RAISE NOTICE '✅ Tabela public.users removida';
    ELSE
      RAISE NOTICE 'A tabela tem % registros.', public_users_count;
      RAISE NOTICE 'Vamos renomeá-la para public_users_old...';
      ALTER TABLE IF EXISTS public.users RENAME TO public_users_old;
      RAISE NOTICE '✅ Tabela public.users renomeada para public_users_old';
    END IF;
    RAISE NOTICE '========================================';
  ELSE
    RAISE NOTICE '✅ Não existe tabela public.users - OK';
  END IF;
END $$;

-- PASSO 2: REMOVER TODAS AS FOREIGN KEYS
DO $$
DECLARE
...
-- PASSO 5: CRIAR FUNÇÃO RPC create_user_profile
  ...
  CASE 
    WHEN EXISTS (
      ...
      WHERE relnamespace = 'public'::regnamespace
        AND relname = 'users'
        AND relkind = 'r'
    ) THEN '⚠️ Tabela public.users: AINDA EXISTE (PROBLEMA!)'
    ELSE '✅ Tabela public.users: NÃO EXISTE (OK)'
